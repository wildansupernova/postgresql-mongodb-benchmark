...
public class PostgresMultiTableOps implements DatabaseOperations {
        ...
        RetryUtil.executeVoidWithRetry(() -> {
            ...
            try {
                ...
                try (PreparedStatement pstmt = conn.prepareStatement(updateItemSql)) {
                    for (Item item : items) {
                        pstmt.setLong(1, item.getPrice());
                        pstmt.setLong(2, item.getQuantity());
            ...
            try {
                ...
                try (PreparedStatement pstmt = conn.prepareStatement(itemSql)) {
                    for (int i = 0; i < 5; i++) {
                        ...
                        Item newItem = new Item(itemId, orderId, "product_" + (currentSize + i + 1),
                                "New Product " + (currentSize + i + 1), price, quantity);
                        items.add(newItem);
                    }
                    pstmt.executeBatch();
                }
    ...
    public Order query(String orderId) throws Exception {
        return RetryUtil.executeWithRetry(() -> {
            ...
            String orderSql = "SELECT * FROM " + ORDERS_TABLE + " WHERE order_id = ?";
            Order order = null;
            ...
            String itemsSql = "SELECT * FROM " + ITEMS_TABLE + " WHERE order_id = ?";
